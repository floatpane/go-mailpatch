...
```bash
git clone https://github.com/floatpane/go-mailpatch.git
cd go-mailpatch
go mod tidy
```
...
```
feat(diff): track file mode changes
fix(parse): strip RFC 2047 encoded-words from the subject prefix
docs: document the mbox series-grouping rules
```
...
- A minimal reproducer (the patch/mbox message that triggers the bug, a `Parse`/`ParseDiff` snippet, or a failing test)
